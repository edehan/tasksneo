...
export function TaskImportDialog({
  ...
  useEffect(() => {
    ...
    listTaskImportCandidates({ sort })
      ...
      .catch((err) => {
        const message =
          err instanceof ApiError ? err.message : t("failedLoad");
        toast.error(message);
      })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl gap-0 overflow-hidden p-0">
          ...
          <aside className="min-h-0 overflow-y-auto bg-surface-subtle/40 p-4">
            {selectedTask ? (
              <div className="space-y-4">
                ...
                <div>
                  <p className="text-label-upper mb-2">{t("attachmentSummary")}</p>
                  {detailLoading ? (
        ...
        <DialogFooter className="border-t border-border px-6 py-4">
          ...
          >
            {importing && (
              <Loader2 size={14} strokeWidth={2} className="mr-1.5 animate-spin" />
            )}
            {t("import")}
          </Button>
        </DialogFooter>

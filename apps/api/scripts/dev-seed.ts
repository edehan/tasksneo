...
async function upsertClass(ownerId: string, input: { name: string; inviteCode: string; color: string; description?: string }) {
...
async function upsertTask(input: {
  ...
  const data = {
    ...
    publishedAt: (input.isPublished ?? true) ? new Date(input.startAtIso) : null,
    deletedAt: null,
  };
...
async function main() {
  ...
  for (const userId of [userA.id, userB.id, userC.id]) {
    await ensureMember(classTourism.id, userId, userId === userA.id ? ClassRole.OWNER : ClassRole.MEMBER);
  }
...
3. 与周边交通部门联动，优化抵离时段分布`,
    score: 88,
    reviewNote: '数据引用规范，分析逻辑清晰。建议增加一手调研数据佐证，结论部分可更有针对性。',
    reviewerId: userA.id,
  });
...
引入 AI 客流预测系统，提前 48 小时向旅行社推送限流预警，协助旅行社动态调整行程安排。`,
    score: 82,
    reviewNote: '选取景区有新意，长隆数据部分缺乏来源说明。整体结构完整，语言流畅。',
    reviewerId: userA.id,
  });
...
这里也是摄影爱好者的天堂，建议大家在日落后的蓝调时刻，将相机架设在湖边栏杆处，捕捉城市灯光与湖月交织的绝美一幕。`,
  ...
  for (const userId of [userA.id, userB.id, userC.id]) {
    await ensureMember(classLiterature.id, userId, userId === userA.id ? ClassRole.OWNER : ClassRole.MEMBER);
  }
...
综上，余华的叙事视角选择是一种伦理决策：不以作者身份消费苦难，而是让苦难以最日常的声音缓缓流出。`,
    score: 93,
    reviewNote: '分析深入，引文精准，对第一人称悖论的阐发尤为精彩。语言略可再凝练。优秀。',
    reviewerId: userA.id,
  });
...
我离开成都时，行李箱里不知怎么混进了一小包花椒。在家里打开箱子的那一刻，成都扑面而来。`,
    score: 91,
    reviewNote: '细节生动，三种气味层次分明，结尾余韵悠长。"一碗拌匀的红油"是全文最好的一句，克制而有力。',
    reviewerId: userA.id,
  });
  ...
  const classFinance = await upsertClass(userA.id, {
    ...
    color: '#16a34a',
    description: '股票、债券、外汇市场基础理论与量化分析实践，配合 Python 工具链。',
  });

  for (const userId of [userA.id, userB.id, userC.id]) {
    await ensureMember(classFinance.id, userId, userId === userA.id ? ClassRole.OWNER : ClassRole.MEMBER);
  }
...
建议探索"高频已实现波动率 + 期权隐含波动率 + 情绪指数"三因子融合模型，同时引入涨跌停触板率作为极端情绪修正项，以提升 A 股特殊制度背景下的预测效度。`,
    score: 85,
    reviewNote: '结构清晰，对三点差异的分析到位。改进方向有创意，但建议进一步查阅已有文献，避免重复已有成果。',
    reviewerId: userA.id,
  });
...
鉴于 A 股特殊性，可考虑引入基于历史高频数据的 HAR-RV 模型（Heterogeneous Autoregressive Model of Realized Volatility），其对短中长期波动率的分解思路更契合 A 股投资者的异质性特征。`,
  ...
  for (const userId of [userA.id, userB.id, userC.id]) {
    await ensureMember(classHistory.id, userId, userId === userA.id ? ClassRole.OWNER : ClassRole.MEMBER);
  }
...
这次采访让我意识到历史教科书里的"改革开放"是一个整体性叙事，但对于每一个普通人而言，它是以非常具体的方式降临的——一批确良布、一扇被挤碎的玻璃、一把柜台钥匙。外婆的经历提醒我，历史不只存在于政策文件里，也存在于那些被放进铁盒子保存的布票里。`,
    score: 95,
    reviewNote: '极为出色的口述历史作品。细节真实，叙事克制而有力，"空了一块"的结尾令人动容。"确良布"作为时代符号的使用尤为精准。强烈推荐作为本学期范文。',
    reviewerId: userA.id,
  });
...
受限于视频采访，现场感有所缺失，无法记录受访者的肢体语言与环境细节，这是本次采访最大的遗憾。但王叔叔的语言本身极具感染力，"躺平会死"这句话让我久久无法释怀——它是一个时代的注脚，也是对当下某种情绪的无声回应。`,
    score: 88,
    reviewNote: '主题选取有现实关怀，"躺平会死"的对比很有张力。建议下次尽量争取线下采访，补充环境与非语言细节。整体完成质量良好。',
    reviewerId: userA.id,
  });

...
export async function setTaskDraftMarkdown(taskId: string, markdown: string) {
  try {
    ...
    await redis.set(getTaskDraftMarkdownKey(taskId), markdown, 'EX', TASK_DRAFT_MARKDOWN_TTL_SECONDS);
  } catch {
    // Redis should not block the main flow.
  }
}

export async function getTaskDraftMarkdown(taskId: string): Promise<string | null> {
  try {
    const redis = getRedisClient();

...
export async function getMembershipOrThrow(classId: string, userId: string): Promise<MembershipContext> {
...
export function requireOwnerOrAdmin(membership: MembershipContext) {
  if (membership.role !== ClassRole.OWNER && membership.role !== ClassRole.ADMIN) {

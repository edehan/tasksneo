...
describe("TaskFlow API e2e", () => {
	...
	it("lists and imports published tasks for managed classes", async () => {
		...
		const olderTask = await requestJson(
			app,
			`/classes/${classRow.id}/tasks`,
			{
				method: "POST",
				headers: authHeader(owner.token),
				body: JSON.stringify({
		...
		const newerTask = await requestJson(
			app,
			`/classes/${classRow.id}/tasks`,
			{
				method: "POST",
		...
		const memberCandidates = await requestJson(app, "/tasks/import-candidates", {
			headers: authHeader(member.token),
		});
		expect(memberCandidates.response.status).toBe(200);
		expect((memberCandidates.body as { tasks: unknown[] }).tasks).toHaveLength(0);

		const draft = await requestJson(app, `/classes/${classRow.id}/tasks/drafts`, {
			method: "POST",
			headers: authHeader(owner.token),

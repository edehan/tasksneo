...
type SubmissionFilter = "all" | "submitted" | "unsubmitted" | "ungraded" | "graded" | "exemplary";
...
export function registerSubmissionTools(
	...
	server.registerTool("list_submissions", {
		...
		inputSchema: {
			...
			filter: z
				.enum(["all", "submitted", "unsubmitted", "ungraded", "graded", "exemplary"])
				.default("all")
				.describe("Filter: all, submitted, unsubmitted, ungraded, graded, or exemplary"),
		},
		annotations: {
			readOnlyHint: true,
	...
	server.registerTool("grade_submission", {
		...
		inputSchema: {
			...
			score: z
				...
				.nullable()
				.optional()
				.describe("Score as a string (e.g. '85', '92.5'). Set null to clear."),
			reviewNote: z
				.string()
				.nullable()
				.optional()
	...
	server.registerTool("toggle_exemplary", {
		description:

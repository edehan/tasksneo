...
export function registerFileTools(
	...
	server.registerTool(
		...
		{
			description:
				"Get presigned download URLs for attachments. Use sync_taskflow_task instead when the user wants a task and its submitted student work saved into a local folder. Use this URL-only tool when you specifically need temporary links for another HTTP tool (WebFetch, curl, etc.) — URLs work without auth headers and expire in 5 minutes.\n\nVisibility rules: when you pass a taskId, members only receive task attachments marked visible. Admins can receive hidden task attachments as well. If you already have specific fileKeys, download access still follows the server's underlying permission checks.\n\nThree input modes (provide exactly one):\n- taskId: returns URLs for task attachments the current caller is allowed to access\n- taskId + submissionId: returns URLs for all files a student uploaded to a submission\n- fileKeys: returns URLs for specific fileKeys you already know about\n\nThe tool presigns URLs in parallel, so large batches are fast. Individual failures are reported in the `failed` array without breaking the rest of the response. Max 50 fileKeys per call.",
			inputSchema: {
				taskId: z
					.string()
					.uuid()
